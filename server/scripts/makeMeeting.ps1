    ...
    foreach ($impdUser in $users) {        
        ...
        foreach ($apApointment in $frCalendarResult.Items) {
            ...
            if (($apApointment.Subject -eq $Subject_D) -and ($apApointment.start -eq $start)) {
                ...
                $appointment.Subject + ";" + $appointment.Body + ";" + $appointment.Start + ";" + $appointment.End + ";" + $appointment.IsAllDayEvent + ";" + $appointment.Categories + ";" + $appointment.Location + ";" + $appointment.LegacyFreeBusyStatus | Out-File $Path$outfileAppointM -Append
